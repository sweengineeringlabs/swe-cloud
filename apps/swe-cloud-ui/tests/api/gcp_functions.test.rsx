//! Tests for Gcp Cloud Functions client.
//!
//! Generated by `rsc scaffold services`

use rustscript::test::*;
use crate::api::gcp::functions::*;
use crate::api::mocks::gcp::FunctionsClientMock;
...
#[tokio::test]
async fn functions_call_function_with_mock() {
    let mock = FunctionsClientMock::new()
        .with_response("CallFunction", CallFunctionResponse::default());

    let request = CallFunctionRequest::default();
    let result = mock.call_function(request).await;

    assert!(result.is_ok());
    assert!(mock.was_called("CallFunction"));
}
