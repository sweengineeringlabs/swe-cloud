...
use cloudkit_spi::common::{CloudResult, CloudError};
...
impl MetricsService for CloudWatchMetrics {
    ...
    async fn get_metric_data(
        ...
        if let Some(results) = resp.metric_data_results().get(0) {
            for (v, t) in results.values().iter().zip(results.timestamps().iter()) {
                data_points.push(cloudkit::api::MetricDataPoint {
                    timestamp: chrono::DateTime::<chrono::Utc>::from_timestamp(t.secs(), 0).unwrap_or_default(),
                    value: *v,
                    unit: cloudkit::api::MetricUnit::None, // CloudWatch doesn't always return unit in GetMetricData
                });
            }
        }
    ...
    async fn put_alarm(&self, namespace: &str, config: AlarmConfig) -> CloudResult<()> {
        ...
        req = req.comparison_operator(match config.comparison {
            cloudkit::api::ComparisonOperator::GreaterThan => aws_sdk_cloudwatch::types::ComparisonOperator::GreaterThanThreshold,
            cloudkit::api::ComparisonOperator::GreaterThanOrEqual => aws_sdk_cloudwatch::types::ComparisonOperator::GreaterThanOrEqualToThreshold,
            cloudkit::api::ComparisonOperator::LessThan => aws_sdk_cloudwatch::types::ComparisonOperator::LessThanThreshold,
            cloudkit::api::ComparisonOperator::LessThanOrEqual => aws_sdk_cloudwatch::types::ComparisonOperator::LessThanOrEqualToThreshold,
        });
