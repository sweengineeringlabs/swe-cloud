...
use tracing::info;
use data_plane::{Config, StorageEngine};
...
pub async fn start_server(config: Config) -> Result<()> {
    ...
    info!("  ✓ Lambda");
    info!("─────────────────────────────────────────");
    // Only display data dir if not using in-memory (Storage check needed, but config has it?)
    // data_dir in Config usually.
    ...
    axum::serve(listener, app).await
         .map_err(|e| data_plane::error::EmulatorError::Io(e))?; // axum error to io error? 
         // Actually axum::serve returns io::Result.
