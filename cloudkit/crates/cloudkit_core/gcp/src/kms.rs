...
use cloudkit_api::{
    ...
    KeyMetadata, SigningAlgorithm, KeyUsage,
};
...
impl KeyManagement for GcpKms {
    async fn create_key(&self, options: CreateKeyOptions) -> CloudResult<KeyMetadata> {
        ...
        let purpose = match options.usage {
            ...
            KeyUsage::KeyAgreement => "ASYMMETRIC_SIGN", // ECDH
            KeyUsage::GenerateVerifyMac => "MAC",
        };
...
mod tests {
    ...
    async fn test_kms_flow() {
        ...
        println!("Created key: {}", key_metadata.key_id);
        ...
        println!("Encrypted {} bytes", encrypt_result.ciphertext.len());
